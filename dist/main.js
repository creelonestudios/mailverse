...
secure: if (getConfig("enable_pop3s", false) || getConfig("enable_smtps", false)) {
    ...
    if (getConfig("enable_smtps", false))
        new SMTPServer(getConfig("smtps_port", 465), true, tlsKey, tlsCert); // Port 25 for regular SMTP, 465 for SMTPS
}
if (getConfig("enable_smtp", true))
    new SMTPServer(getConfig("smtp_port", 25), false); // Port 25 for regular SMTP, 465 for SMTPS
